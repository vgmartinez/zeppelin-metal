...
import org.apache.zeppelin.users.User;
import org.apache.zeppelin.users.UserPermission;
...
public class NotebookServer extends WebSocketServlet implements
  ...
  List<NotebookSocket> connectedSockets = new LinkedList<>();
  ...
  public void subscribe(final List<Note> notes) throws InterruptedException {
    final Notebook notebook = notebook();
    String host = notebook.getConf().getRedisDB();
    
    final JedisPoolConfig poolConfig = new JedisPoolConfig();
    final JedisPool jedisPool = new JedisPool(poolConfig, host, 6379, 0);
    final Jedis subscriberJedis = jedisPool.getResource();
    publisherJedis = jedisPool.getResource();
    
    final String [] channel = new String[notes.size() + 1];
    for (int i = 0; i < notes.size(); i++) {
      channel[i] = notes.get(i).getKey();
    }
    
    LOG.info("USER: " + notebook.getUserName());
    channel[notes.size()] = notebook.getUserName();
        ...
        try {
          subscriberJedis.subscribe(new JedisPubSub() {
            ...
            public void onMessage(String channel, String message) {
              ...
              try {
                if (channel.equals(notebook.getUserName())) {
                  broadcastNoteList();
                } else {
                  sendNoteToFront(channel, message);
                }
    ...
    try {
      ...
      switch (messagereceived.op) {
          ...
          case GET_NOTE:
            ...
            break;
          case SET_GIST:
            setGist(conn, notebook, messagereceived);
            break;
          case NEW_NOTE:
          ...
          case NOTE_UPDATE:
            ...
            break;
          case LIST_USERS:
            listUsers(conn, notebook, messagereceived);
            break;
          case COMPLETION:
  ...
  private void broadcastNoteList() {
    ...
    for (Note note : notes) {
      ...
      //info.put("owner", note.isMy(note.getOwner()) + "");
      notesInfo.add(info);
    }
  ...
  private void sendNote(NotebookSocket conn, Notebook notebook,
    ...
    if (note != null) {
      ...
      List<User> users = notebook.listUsers();
      List<UserPermission> usersPermission = notebook.listPermission(noteId);
      
      conn.send(serializeMessage(new Message(OP.USER).put("users", users)));
      conn.send(serializeMessage(new Message(OP.PERMISSION).put("permissions", usersPermission)));
      
    }
  }
  
  public void sendNoteToFront(String channel, String user) throws IOException {
    Notebook notebook = notebook();
    Note note = notebook.loadNoteFromS3(channel); //getNote(getOpenNoteId(conn));
    
    if (notebook.getUserName().equals(user)) {
      LOG.info(notebook.getUserName());
      return;
    } else {
      if (note != null) {
        broadcastNote(note);
  ...
  private void updateNote(WebSocket conn, Notebook notebook, Message fromMessage)
    ...
    if (note != null) {
      ...
      try {
        notebook.publish(note, publisherJedis);
      } catch (InterruptedException e) {
        e.printStackTrace();
      }
  ...
  private void listUsers(NotebookSocket conn, Notebook notebook, Message messagereceived) 
      throws IOException {
    List<User> users = notebook.listUsers();
    conn.send(serializeMessage(new Message(OP.USER).put("users", users)));
  }
  ...
  public void setGist(WebSocket conn, Notebook notebook, Message fromMessage) {
    String noteId = (String) fromMessage.get("id");
    String gistId = (String) fromMessage.get("gist");
    if (noteId == null) {
      return;
    }
    LOG.info("noteId " + noteId);
    LOG.info("gistId " + gistId);
    notebook.setGistId(noteId, gistId);;
  }
